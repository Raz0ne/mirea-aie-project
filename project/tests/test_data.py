"""Тесты генератора синтетических данных и загрузчика датасета."""
...
def test_synthetic_positive_rate_realistic():
    """Доля положительного класса должна лежать в правдоподобном диапазоне (1%-25%)."""
    df = generate(n_rows=20_000, random_state=0)
    rate = df[TARGET].mean()
    assert 0.01 <= rate <= 0.25, f"нереалистичная доля положительного класса: {rate:.3f}"
