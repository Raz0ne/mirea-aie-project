...
def head(
    path: str = typer.Argument(..., help="Путь к CSV-файлу."),
    n: int = typer.Option(5, "--n", help="Количество первых строк для вывода."),
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
) -> None:
    """
    ...
    if n < 0:
        raise typer.BadParameter("Параметр --n должен быть неотрицательным числом")
    
    if n == 0:
        typer.echo("Запрошено 0 строк. Ничего не выводится.")
        return
    ...
    total_rows = len(df)
    if n < total_rows:
        typer.echo(f"\n... (показано {n} из {total_rows} строк)")
...
def report(
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        # Информация о настройках отчёта
