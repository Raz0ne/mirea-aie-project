...
Эндпоинт принимает CSV-файл и возвращает полный набор булевых флагов качества данных, включая все эвристики из `compute_quality_flags` (например, `has_constant_columns`, `has_high_cardinality_categoricals`, `has_many_zero_values` и т.п.).
...
Ответ содержит только булевы флаги качества без интегрального скора, что полезно для детального анализа проблем датасета.
