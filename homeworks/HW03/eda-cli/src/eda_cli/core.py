...
def compute_quality_flags(
    ...
    cardinality_threshold_relative = 0.5
    cardinality_threshold_absolute = 100
    
    for col in summary.columns:
        if not col.is_numeric:
            threshold = max(
                int(summary.n_rows * cardinality_threshold_relative),
                cardinality_threshold_absolute
            )
    ...
    if df is not None:
        for col in summary.columns:
            if col.is_numeric and col.name in df.columns:
                numeric_col = df[col.name]
                non_null_count = col.non_null
                if non_null_count > 0:
                    zero_count = int((numeric_col == 0).sum())
                    zero_share = zero_count / non_null_count
